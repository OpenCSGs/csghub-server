...
func (s *serviceComponentImpl) RunServiceMultiHost(ctx context.Context, req types.SVCRequest) error {
	...
	if err == nil {
		slog.Warn("service already exists,delete it first", slog.String("srv_name", req.SrvName), slog.Any("image_id", req.ImageID))
		return fmt.Errorf("service %v already exists,delete it first. error: %w ", req.SrvName, err)
	}
	// create lws service
	...
	if err != nil {
		return fmt.Errorf("failed to create lws service, error: %w", err)
	}
	slog.Info("create lws service success", slog.String("srv_name", req.SrvName))
...
func (s *serviceComponentImpl) GenerateNginxService(ctx context.Context, cluster cluster.Cluster, req types.SVCRequest) (*v1.Service, error) {
	...
	containerImg := path.Join(s.modelDockerRegBase, "nginx:latest")
	...
	proxyConfig := fmt.Sprintf(`server {
	  ...
	  }`, appPort, req.SrvName+LWSSuffix, appPort)
		...
		ProbeHandler: corev1.ProbeHandler{
			Exec: &corev1.ExecAction{
				Command: []string{
					"curl",
					"-f",
					fmt.Sprintf("http://%s:%s/health", req.SrvName+LWSSuffix, appPort),
				},
			},
		},
...
func (s *serviceComponentImpl) GenerateLWSService(ctx context.Context, cluster cluster.Cluster, request types.SVCRequest) (*lwsv1.LeaderWorkerSet, error) {
	...
	lws := &lwsv1.LeaderWorkerSet{
		ObjectMeta: metav1.ObjectMeta{
			Name:      request.SrvName + LWSSuffix,
				...
				LeaderTemplate: &corev1.PodTemplateSpec{
					ObjectMeta: metav1.ObjectMeta{
						Labels: map[string]string{
							...
							"serving.knative.dev/service": request.SrvName,
						},
					},
					Spec: corev1.PodSpec{
						HostNetwork:      true,
