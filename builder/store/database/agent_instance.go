...
type AgentTemplate struct {
	...
	IsPinned    bool           `bun:",scanonly" json:"is_pinned"` // Whether the template is pinned (from LEFT JOIN)
	PinnedAt    *time.Time     `bun:",scanonly" json:"pinned_at"` // When the template was pinned (from LEFT JOIN)
	times
}
...
type AgentInstanceStore interface {
	...
	IsInstanceNameExists(ctx context.Context, userUUID string, instanceName string) (bool, error)
}
...
func (s *agentTemplateStoreImpl) applyAgentTemplateFilters(query *bun.SelectQuery, filter types.AgentTemplateFilter) *bun.SelectQuery {
	...
	if filter.Search != "" {
		...
		query = query.Where("LOWER(at.name) LIKE LOWER(?) OR LOWER(at.description) LIKE LOWER(?)", searchPattern, searchPattern)
	}
...
func (s *agentTemplateStoreImpl) ListByUserUUID(ctx context.Context, userUUID string, filter types.AgentTemplateFilter, per int, page int) ([]AgentTemplate, int, error) {
	...
	query := s.db.Core.NewSelect().
		TableExpr("agent_templates AS at").
		ColumnExpr("at.*").
		...
		Join(`
         LEFT JOIN agent_user_preferences pin_pref
          ...
          AND pin_pref.entity_id = CAST(at.id AS TEXT)
     `, userUUID, types.AgentUserPreferenceActionPin, types.AgentUserPreferenceEntityTypeAgentTemplate).
	...
	err = query.
		OrderExpr("pin_pref.created_at DESC NULLS LAST, at.updated_at DESC").
		Limit(per).
		Offset((page-1)*per).
		Scan(ctx, &templates)
	if err != nil {
...
func (s *agentInstanceStoreImpl) applyAgentInstanceFilters(query *bun.SelectQuery, userUUID string, filter types.AgentInstanceFilter) *bun.SelectQuery {
	filter.Search = strings.TrimSpace(filter.Search)
	if filter.Search != "" {
		searchPattern := "%" + filter.Search + "%"
		query = query.Where("LOWER(ai.name) LIKE LOWER(?) OR LOWER(ai.description) LIKE LOWER(?)", searchPattern, searchPattern)
	}

	if filter.Type != "" {
		query = query.Where("ai.type = ?", filter.Type)
	}

	if filter.TemplateID != nil {
		query = query.Where("ai.template_id = ?", *filter.TemplateID)
	}

	if filter.BuiltIn != nil {
		query = query.Where("ai.built_in = ?", *filter.BuiltIn)
	}

	if filter.Public != nil {
		query = query.Where("ai.public = ?", *filter.Public)
	}

	if filter.Editable != nil {
		if *filter.Editable {
			query = query.Where("ai.user_uuid = ?", userUUID)
		} else {
			query = query.Where("ai.user_uuid != ?", userUUID)
		}
	}
...
func (s *agentInstanceStoreImpl) ListByUserUUID(ctx context.Context, userUUID string, filter types.AgentInstanceFilter, per int, page int) ([]AgentInstance, int, error) {
	...
	query = s.applyAgentInstanceFilters(query, userUUID, filter)
	...
	err = query.
		OrderExpr("pin_pref.created_at DESC NULLS LAST, ai.updated_at DESC").
		Limit(per).
		Offset((page-1)*per).
		Scan(ctx, &instances)
	if err != nil {
...
// It also soft-deletes all related sessions, session histories, and tasks
func (s *agentInstanceStoreImpl) Delete(ctx context.Context, id int64) error {
	return s.db.Core.RunInTx(ctx, nil, func(ctx context.Context, tx bun.Tx) error {
		// First, get all session IDs for this instance
		...
		// Soft-delete all session histories for these sessions
		if len(sessionIDs) > 0 {
			_, err = tx.NewDelete().
			...
			if err != nil {
				return errorx.HandleDBError(err, map[string]any{
					"instance_id": id,
					"operation":   "delete_session_histories",
				})
			}
		}

		// Soft-delete all sessions for this instance
		_, err = tx.NewDelete().
			Model((*AgentInstanceSession)(nil)).
			Where("instance_id = ?", id).
			Exec(ctx)
		...
		// Soft-delete all tasks for this instance
		_, err = tx.NewDelete().
			Model((*AgentInstanceTask)(nil)).
			Where("instance_id = ?", id).
			Exec(ctx)
		if err != nil {
			return errorx.HandleDBError(err, map[string]any{
		...
		// Finally, soft-delete the instance itself
...
func (s *agentInstanceStoreImpl) IsInstanceNameExists(ctx context.Context, userUUID string, instanceName string) (bool, error) {
	exists, err := s.db.Core.NewSelect().
		Model((*AgentInstance)(nil)).
		Where("user_uuid = ? AND name = ?", userUUID, instanceName).
		Exists(ctx)
	if err != nil {
		return false, errorx.HandleDBError(err, map[string]any{
			"user_uuid":     userUUID,
