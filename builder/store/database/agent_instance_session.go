...
func (s *agentInstanceSessionStoreImpl) Delete(ctx context.Context, id int64) error {
	return s.db.Core.RunInTx(ctx, nil, func(ctx context.Context, tx bun.Tx) error {
		// Load session uuid for cascading deletes (shares table uses session_uuid)
		var sessionUUID string
		err := tx.NewSelect().
			Model((*AgentInstanceSession)(nil)).
			ColumnExpr("uuid").
			Where("id = ?", id).
			Scan(ctx, &sessionUUID)
		if err != nil {
			return errorx.HandleDBError(err, map[string]any{
				"session_id": id,
				"operation":  "select_session_uuid",
			})
		}

		// Delete all session share records
		_, err = tx.NewDelete().
			Model((*AgentInstanceSessionShare)(nil)).
		...
		if err != nil {
			return errorx.HandleDBError(err, map[string]any{
				...
				"operation":    "delete_session_shares",
			})
		}
...
func (s *agentInstanceSessionHistoryStoreImpl) ListBySessionID(ctx context.Context, sessionID int64, maxTurn *int64) ([]AgentInstanceSessionHistory, error) {
	...
	if maxTurn != nil {
		query = query.Where("turn <= ?", *maxTurn)
	}
	err := query.Order("turn ASC", "request DESC").Scan(ctx, &histories)
	if err != nil && !errors.Is(err, sql.ErrNoRows) {
		return nil, errorx.HandleDBError(err, map[string]any{
