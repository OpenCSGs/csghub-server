...
type XnetMigrationTaskStore interface {
	CreateXnetMigrationTask(ctx context.Context, repoID int64, lastMessage string) error
...
func (s *XnetMigrationTaskStoreImpl) CreateXnetMigrationTask(ctx context.Context, repoID int64, lastMessage string) error {
	_, err := s.db.Operator.Core.NewInsert().Model(&XnetMigrationTask{
		RepositoryID: repoID,
