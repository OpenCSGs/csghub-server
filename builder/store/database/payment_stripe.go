...
type PaymentStripe struct {
	...
	OpUUID             string    `bun:",nullzero" json:"op_uuid"`
	times
}
...
func (s *paymentStripeStoreImpl) List(ctx context.Context, req *types.StripeSessionListReq) (*PaymentStripeListResult, error) {
	...
	if len(req.QueryTargetUUID) > 0 {
		q = q.Where("user_uuid = ?", req.QueryTargetUUID)
	}
	...
	if len(req.QueryTargetUUID) > 0 {
		sumQuery = sumQuery.Where("user_uuid = ?", req.QueryTargetUUID)
	}
