...
func (s *agentInstanceTaskStoreImpl) buildListQuery(taskType types.AgentTaskType, userUUID string, filter types.AgentTaskFilter) (*bun.SelectQuery, error) {
	...
	query = query.ColumnExpr(fmt.Sprintf(`ait.id,
		...
		ait.instance_id,
		ait.session_uuid,
		(pin_pref.id IS NOT NULL) AS is_pinned,
		pin_pref.created_at AS pinned_at,
		ait.updated_at,
        ...
        LEFT JOIN agent_user_preferences pin_pref
         ...
         AND pin_pref.entity_id = CAST(ait.id AS TEXT)
    `, userUUID, types.AgentUserPreferenceActionPin, types.AgentUserPreferenceEntityTypeAgentTask)

	query = query.Where("ait.deleted_at IS NULL").
		Where("ait.user_uuid = ?", userUUID).
...
func (s *agentInstanceTaskStoreImpl) ListTasks(ctx context.Context, userUUID string, filter types.AgentTaskFilter, per int, page int) ([]types.AgentTaskListItem, int, error) {
	type taskRow struct {
		...
		PinnedAt    *time.Time `bun:"pinned_at"`
	...
	// Sort by pinned_at DESC NULLS LAST, then updated_at DESC
	sort.Slice(allRows, func(i, j int) bool {
		// If one is pinned and the other is not, pinned comes first
		if allRows[i].IsPinned && !allRows[j].IsPinned {
			return true
		}
		if !allRows[i].IsPinned && allRows[j].IsPinned {
			return false
		}
		// If both pinned or both not pinned, sort by pinned_at if pinned, otherwise by updated_at
		if allRows[i].IsPinned && allRows[j].IsPinned {
			if allRows[i].PinnedAt != nil && allRows[j].PinnedAt != nil {
				return allRows[i].PinnedAt.After(*allRows[j].PinnedAt)
			}
			if allRows[i].PinnedAt != nil {
				return true
			}
			if allRows[j].PinnedAt != nil {
				return false
			}
		}
		// Both not pinned, sort by updated_at
