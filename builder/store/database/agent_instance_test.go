...
func TestAgentInstanceStore_CRUD(t *testing.T) {
	...
	*instance = *createdInstance
	...
	require.NoError(t, err)
	require.Len(t, instances, 7) // 1 user instance + 6 system instances (all public)
	...
	instances, total, err = store.ListByUserUUID(ctx, userUUID, types.AgentInstanceFilter{}, 10, 1)
	require.NoError(t, err)
	require.Len(t, instances, 6) // Only 6 system instances (all public), deleted instance should not appear
...
func TestAgentInstanceStore_ListByUserUUID_WithPublicInstances(t *testing.T) {
	...
	require.NoError(t, err)
	require.Len(t, instances, 8) // 2 user instances + 6 system instances (all public)
	...
	require.NoError(t, err)
	require.Len(t, instances, 8) // public instance from user1 + private instance from user2 + 6 system instances
	require.Equal(t, 8, total)
}
...
func TestAgentInstanceStore_NotFound(t *testing.T) {
	...
	require.NoError(t, err)
	require.Len(t, instances, 6) // Only system instances (all public)
	require.Equal(t, 6, total)
...
func TestAgentInstanceStore_ListByUserUUID_WithFilters(t *testing.T) {
	...
	require.Equal(t, 9, total)   // But total should be 9 (3 user + 6 system)
	...
	require.Equal(t, 9, total)   // Total should still be 9 (3 user + 6 system)
}
...
func TestAgentInstanceStore_Delete_WithCascadingSoftDelete(t *testing.T) {
	...
	histories1, err := historyStore.ListBySessionID(ctx, createdSession1.ID, nil)
	require.NoError(t, err)
	require.Len(t, histories1, 2)

	histories2, err := historyStore.ListBySessionID(ctx, createdSession2.ID, nil)
	require.NoError(t, err)
