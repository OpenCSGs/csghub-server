...
func TestAccountSyncQuotaStatementStore_CountByRepoPath(t *testing.T) {
	db := tests.InitTestDB()
	defer db.Close()
	ctx := context.TODO()

	// Setup: allow repo creation for multiple users
	_, err := db.Core.NewInsert().Model(&database.AccountSyncQuota{
		UserID:         100,
		RepoCountLimit: 10,
		RepoCountUsed:  0,
	...
	// repo-a appears in 2 records
	count, err := store.CountByRepoPath(ctx, "ns1/repo-a")
	require.Nil(t, err)
	require.Equal(t, 2, count)

	// non-existent path
	count, err = store.CountByRepoPath(ctx, "ns1/nonexistent")
	require.Nil(t, err)
