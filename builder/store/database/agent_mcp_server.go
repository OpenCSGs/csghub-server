...
type AgentMCPServerView struct {
	...
	IsPinned     bool           `bun:",scanonly" json:"is_pinned"` // Whether the MCP server is pinned (from LEFT JOIN)
	PinnedAt     *time.Time     `bun:",scanonly" json:"pinned_at"` // When the MCP server was pinned (from LEFT JOIN)
	times
}

// AgentMCPServerAllView represents a row from the agent_mcp_server_all_views view.
// This view is NOT user-specific and lists all MCP servers across:
// - built-in no-install (mcp_resources.need_install=false)
// - built-in need-install overrides (agent_mcp_server_configs + mcp_resources)
// - user-added (agent_mcp_servers)
type AgentMCPServerAllView struct {
	ID          string         `bun:",pk" json:"id"` // "builtin:{id}" or "user:{id}"
	Name        string         `json:"name"`
	Description string         `json:"description"`
	UserUUID    string         `json:"user_uuid"`
...
type AgentMCPServerStore interface {
	...
	// ListAll lists MCP servers across all users with pagination.
	ListAll(ctx context.Context, per int, page int) ([]AgentMCPServerAllView, int, error)
	IsServerNameExists(ctx context.Context, userUUID string, serverName string) (bool, error)
}
...
// AgentMCPServerStatus represents status for an MCP server
type AgentMCPServerStatus struct {
	ID              int64          `bun:",pk,autoincrement" json:"id"`
	ServerID        string         `bun:",notnull" json:"server_id"` // "builtin:{id}" or "user:{id}"
	UserUUID        string         `bun:",notnull" json:"user_uuid"`
	Status          string         `bun:",notnull" json:"status"` // 'connected', 'disconnected', or 'error'
...
// AgentMCPServerStatusStore provides database operations for AgentMCPServerStatus
type AgentMCPServerStatusStore interface {
	CreateOrUpdate(ctx context.Context, status *AgentMCPServerStatus) error
	FindByServerID(ctx context.Context, serverID string, userUUID string) (*AgentMCPServerStatus, error)
	Delete(ctx context.Context, serverID string, userUUID string) error
}
...
// agentMCPServerStatusStoreImpl is the implementation of AgentMCPServerStatusStore
type agentMCPServerStatusStoreImpl struct {
	db *DB
}
...
func (s *agentMCPServerStoreImpl) FindByID(ctx context.Context, userUUID string, id string) (*AgentMCPServerDetail, error) {
	...
	if isBuiltIn {
		...
		detail.Installed = !detail.NeedInstall || config != nil
		return &detail, nil
	} else {
...
func (s *agentMCPServerStoreImpl) applyAgentMCPServerFilters(query *bun.SelectQuery, filter types.AgentMCPServerFilter) *bun.SelectQuery {
	...
	if filter.Search != "" {
		...
		query = query.Where("LOWER(amsv.name) LIKE LOWER(?)", searchPattern)
	}
	...
	if filter.BuiltIn != nil {
		if *filter.BuiltIn {
			...
			query = query.Where("amsv.built_in = ?", true)
		} else {
			// If built_in is false, show only user's servers (filter by user_uuid)
			query = query.Where("amsv.built_in = ? AND amsv.user_uuid = ?", false, filter.UserUUID)
		}
	} else {
		// Handle user_uuid filter: show both built-in and user's servers (only when built_in is not set)
		if filter.UserUUID != "" {
			query = query.Where("amsv.built_in = ? OR amsv.user_uuid = ?", true, filter.UserUUID)
		}
	}

	// Handle protocol filter
	if filter.Protocol != nil {
		query = query.Where("amsv.protocol = ?", *filter.Protocol)
	}

	// Handle need_install filter
	if filter.NeedInstall != nil {
		query = query.Where("amsv.need_install = ?", *filter.NeedInstall)
	}

	if filter.Status != nil {
		query = query.Where("amsv.status = ?", *filter.Status)
	}

	if filter.Installed != nil {
		query = query.Where("amsv.installed = ?", *filter.Installed)
	}
...
func (s *agentMCPServerStoreImpl) List(ctx context.Context, filter types.AgentMCPServerFilter, per int, page int) ([]AgentMCPServerView, int, error) {
	...
	err := s.db.Core.RunInTx(ctx, nil, func(ctx context.Context, tx bun.Tx) error {
		...
		// Build query with LEFT JOIN to agent_user_preferences
		query := tx.NewSelect().
			TableExpr("agent_mcp_server_views AS amsv").
			ColumnExpr("amsv.*").
         ...
         LEFT JOIN agent_user_preferences pin_pref
          ...
          AND pin_pref.entity_id = amsv.id
     `, filter.UserUUID, types.AgentUserPreferenceActionPin, types.AgentUserPreferenceEntityTypeAgentMCPServer)
		...
		err = query.
			OrderExpr("pin_pref.created_at DESC NULLS LAST, amsv.updated_at DESC").
			Limit(per).
			Offset((page-1)*per).
			Scan(ctx, &servers)
		if err != nil {
...
// ListAll lists MCP servers across all users with pagination.
// Data source: agent_mcp_server_all_views (NOT user-specific).
func (s *agentMCPServerStoreImpl) ListAll(ctx context.Context, per int, page int) ([]AgentMCPServerAllView, int, error) {
	var servers []AgentMCPServerAllView
	var total int

	query := s.db.Core.NewSelect().
		TableExpr("agent_mcp_server_all_views AS amsav").
		ColumnExpr("amsav.*")
	...
	if err != nil {
		return nil, 0, errorx.HandleDBError(err, map[string]any{
			"operation": "count_agent_mcp_servers_all",
		})
	}

	err = query.
		OrderExpr("amsav.updated_at DESC, amsav.id DESC").
		Limit(per).
		Offset((page-1)*per).
		Scan(ctx, &servers)
	if err != nil {
		return nil, 0, errorx.HandleDBError(err, map[string]any{
			"operation": "list_agent_mcp_servers_all",
		})
	}
...
// IsServerNameExists checks if a server name already exists in the agent_mcp_server_views view
// It checks both built-in servers (mcp_resources) and user-created servers (agent_mcp_servers)
func (s *agentMCPServerStoreImpl) IsServerNameExists(ctx context.Context, userUUID string, serverName string) (bool, error) {
	query := s.db.Core.NewSelect().
		TableExpr("agent_mcp_server_views").
		Where("name = ? AND ((built_in = true) OR (built_in = false AND user_uuid = ?))", serverName, userUUID)
...
// NewAgentMCPServerStatusStore creates a new AgentMCPServerStatusStore
func NewAgentMCPServerStatusStore() AgentMCPServerStatusStore {
	return &agentMCPServerStatusStoreImpl{
		db: defaultDB,
	}
}

// NewAgentMCPServerStatusStoreWithDB creates a new AgentMCPServerStatusStore with a specific DB
func NewAgentMCPServerStatusStoreWithDB(db *DB) AgentMCPServerStatusStore {
	return &agentMCPServerStatusStoreImpl{
		db: db,
	}
}
...
// CreateOrUpdate upserts capabilities for an MCP server
func (s *agentMCPServerStatusStoreImpl) CreateOrUpdate(ctx context.Context, status *AgentMCPServerStatus) error {
	status.UpdatedAt = time.Now()
	_, err := s.db.Core.NewInsert().
		Model(status).
		On("CONFLICT (server_id, user_uuid) DO UPDATE").
		Set("status = EXCLUDED.status").
		Set("error = EXCLUDED.error").
		Set("capabilities = EXCLUDED.capabilities").
		Set("last_inspected_at = EXCLUDED.last_inspected_at").
		Set("updated_at = EXCLUDED.updated_at").
		Exec(ctx)
	if err != nil {
		return errorx.HandleDBError(err, map[string]any{
			"server_id": status.ServerID,
			"user_uuid": status.UserUUID,
		})
	}
	return nil
}

// FindByServerID retrieves status by server ID and user UUID
func (s *agentMCPServerStatusStoreImpl) FindByServerID(ctx context.Context, serverID string, userUUID string) (*AgentMCPServerStatus, error) {
	status := &AgentMCPServerStatus{}
	err := s.db.Core.NewSelect().
		Model(status).
		Where("server_id = ? AND user_uuid = ?", serverID, userUUID).
		Scan(ctx, status)
	if err != nil {
		if errors.Is(err, sql.ErrNoRows) {
			return nil, nil
...
// Delete deletes status by server ID and user UUID
func (s *agentMCPServerStatusStoreImpl) Delete(ctx context.Context, serverID string, userUUID string) error {
