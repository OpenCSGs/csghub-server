...
func TestAgentInstanceSessionStore_CRUD(t *testing.T) {
	...
	// Create a share record and ensure it's deleted with session
	shareStore := database.NewAgentInstanceSessionShareStoreWithDB(db)
	shareUUID := uuid.New().String()
	_, err = shareStore.Create(ctx, &database.AgentInstanceSessionShare{
		ShareUUID:   shareUUID,
		UserUUID:    userUUID,
		InstanceID:  instanceID,
	...
	_, err = shareStore.FindByShareUUID(ctx, shareUUID)
	require.Error(t, err)
}
...
func TestAgentInstanceSessionHistoryStore_CRUD(t *testing.T) {
	...
	histories, err := historyStore.ListBySessionID(ctx, createdSession.ID, nil)
	require.NoError(t, err)
...
func TestAgentInstanceSessionHistoryStore_MultipleHistories(t *testing.T) {
	...
	// Test ListBySessionID
	foundHistories, err := historyStore.ListBySessionID(ctx, createdSession.ID, nil)
	require.NoError(t, err)
...
func TestAgentInstanceSessionHistoryStore_ListBySessionID_WithMaxTurn(t *testing.T) {
	...
	maxTurn := int64(1)
	limitedHistories, err := historyStore.ListBySessionID(ctx, createdSession.ID, &maxTurn)
	require.NoError(t, err)
	require.Len(t, limitedHistories, 2)
	require.Equal(t, "User request 1", limitedHistories[0].Content)
	require.Equal(t, "Agent response 1", limitedHistories[1].Content)
}
...
func TestAgentInstanceSessionHistoryStore_ErrorCases(t *testing.T) {
	...
	histories, err := historyStore.ListBySessionID(ctx, 999999, nil)
	require.NoError(t, err)
