...
func TestAccountUsersStore_CheckUserAccount(t *testing.T) {
	db := tests.InitTestDB()
	defer db.Close()
	ctx := context.TODO()
	...
	t.Run("exist-acct-user", func(t *testing.T) {
		au := database.NewAccountUserStoreWithDB(db)
		err := au.Create(ctx, database.AccountUser{
			UserUUID: useruuid,
			Balance:  10,
		...
		err = db.Core.RunInTx(ctx, nil, func(ctx context.Context, tx bun.Tx) error {
			au, err := database.CheckUserAccount(ctx, tx, useruuid)
			require.Equal(t, useruuid, au.UserUUID)
			require.Equal(t, float64(10), au.Balance)
			return err
		})
		require.Nil(t, err)
	})

	t.Run("create-new-acct-user", func(t *testing.T) {
		err := db.Core.RunInTx(ctx, nil, func(ctx context.Context, tx bun.Tx) error {
			au, err := database.CheckUserAccount(ctx, tx, useruuid)
			require.Equal(t, useruuid, au.UserUUID)
			require.Equal(t, float64(10), au.Balance)
