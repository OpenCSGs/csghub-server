...
func (as *accountStatementStoreImpl) deductFeeStatement(ctx context.Context, input AccountStatement) error {
	err := as.db.Operator.Core.RunInTx(ctx, nil, func(ctx context.Context, tx bun.Tx) error {
		...
		if input.Scene == types.SceneCSGShipSub {
			err = UpdateSubscriptionUsage(ctx, tx, input, types.SKUStarship)
			if err != nil {
				return fmt.Errorf("update csgship subscription usage, error: %w", err)
			}
		}

		isCSGHubUsage := utils.IsNeedCheckUserSubscription(input.Scene)
		if isCSGHubUsage {
			err = UpdateSubscriptionUsage(ctx, tx, input, types.SKUCSGHub)
			if err != nil {
				return fmt.Errorf("update csghub subscription usage, error: %w", err)
			}
		}
...
func (as *accountStatementStoreImpl) ListStatementByUserAndSku(ctx context.Context, req types.ActStatementsReq) ([]UserSkuStatement, int, error) {
	...
	selectQuery := as.db.Operator.Core.NewSelect().
		With("grouped_statements", baseQuery).
		TableExpr("grouped_statements").
		Column("user_uuid", "sku_id", "total_value", "total_consumption").
