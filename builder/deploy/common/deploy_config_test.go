...
func TestBuildDeployConfig(t *testing.T) {
	tests := []struct {
		...
		configSetup: func() *config.Config {
			...
			cfg.S3.AccessKeyID = "access-id"
			cfg.S3.AccessKeySecret = "access-secret"
			cfg.S3.Endpoint = "oss.example.com"
			cfg.S3.EnableSSL = true
			cfg.Argo.S3PublicBucket = "video-bucket"
		...
		expectations: func(t *testing.T, result DeployConfig) {
			...
			require.Equal(t, "access-id", result.S3AccessID)
			require.Equal(t, "access-secret", result.S3AccessSecret)
			require.Equal(t, "oss.example.com", result.S3Endpoint)
			require.Equal(t, "video-bucket", result.S3PublicBucket)
			require.True(t, result.S3SSLEnabled)
