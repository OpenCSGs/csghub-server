...
type AgentHubSvcClient interface {
	...
	UpdateAgentInstance(ctx context.Context, userUUID string, contentID string, req *UpdateAgentInstanceRequest) error
...
func (c *AgentHubSvcClientImpl) CreateAgentInstance(ctx context.Context, userUUID string, req *CreateAgentInstanceRequest) (*CreateAgentInstanceResponse, error) {
	...
	path := c.hc.endpoint + "/api/v1/opencsg/flows/"
	hreq, err := http.NewRequestWithContext(ctx, http.MethodPost, path, buf)
	if err != nil {
		return nil, errorx.InternalServerError(err, nil)
	}
	hreq.Header.Set("Content-Type", "application/json")
	hreq.Header.Set("User-UUID", userUUID)
	hreq.Header.Set("Token", c.token)

	hresp, err := c.hc.Do(hreq)
	if err != nil {
		slog.ErrorContext(ctx, "failed to create agent instance in agenthub", "error", err, "rpc_error_ctx", rpcErrorCtx)
		return nil, errorx.RemoteSvcFail(errors.New("failed to create agent instance in agenthub"), rpcErrorCtx)
	}
	defer hresp.Body.Close()
	if hresp.StatusCode != http.StatusOK {
		slog.ErrorContext(ctx, "failed to create agent instance in agenthub", "status_code", hresp.StatusCode, "rpc_error_ctx", rpcErrorCtx)
		return nil, errorx.RemoteSvcFail(errors.New("failed to create agent instance in agenthub"), rpcErrorCtx)
	}
...
func (c *AgentHubSvcClientImpl) GetAgentInstances(ctx context.Context, req *GetAgentInstancesRequest) (GetAgentInstancesResponse, error) {
	...
	buf = bytes.NewBuffer(jsonData)
	path := c.hc.endpoint + "/api/v1/opencsg/flows/query"
	hreq, err := http.NewRequestWithContext(ctx, http.MethodPost, path, buf)
	if err != nil {
		return nil, errorx.InternalServerError(err, nil)
	}
	hreq.Header.Set("Content-Type", "application/json")
	hreq.Header.Set("User-UUID", req.UserUUID)
	hreq.Header.Set("Token", c.token)

	hresp, err := c.hc.Do(hreq)
	if err != nil {
		slog.ErrorContext(ctx, "failed to get agent instance from agenthub", "error", err, "rpc_error_ctx", rpcErrorCtx)
		return nil, errorx.RemoteSvcFail(errors.New("failed to get agent instance from agenthub"), rpcErrorCtx)
	}
	defer hresp.Body.Close()
	if hresp.StatusCode != http.StatusOK {
		slog.ErrorContext(ctx, "failed to get agent instance from agenthub", "status_code", hresp.StatusCode, "rpc_error_ctx", rpcErrorCtx)
		return nil, errorx.RemoteSvcFail(errors.New("failed to get agent instance from agenthub"), rpcErrorCtx)
	}
	body, err := io.ReadAll(hresp.Body)
	if err != nil {
...
func (c *AgentHubSvcClientImpl) DeleteAgentInstance(ctx context.Context, userUUID string, contentID string) error {
	...
	buf := bytes.NewBuffer(jsonData)
	path := c.hc.endpoint + "/api/v1/opencsg/flows/delete"
	hreq, err := http.NewRequestWithContext(ctx, http.MethodPost, path, buf)
	if err != nil {
		return errorx.InternalServerError(err, rpcErrorCtx)
	}
	...
	hresp, err := c.hc.Do(hreq)
	if err != nil {
		slog.ErrorContext(ctx, "failed to delete agent instance in agenthub", "error", err, "rpc_error_ctx", rpcErrorCtx)
...
// PATCH /api/v1/opencsg/flows/{id}
func (c *AgentHubSvcClientImpl) UpdateAgentInstance(ctx context.Context, userUUID string, contentID string, req *UpdateAgentInstanceRequest) error {
	if req == nil {
		return errorx.BadRequest(errors.New("update agent instance request is nil"), nil)
	}
	rpcErrorCtx := map[string]any{
		"user_uuid": userUUID,
		"service":   "agenthub",
		"api":       "/api/v1/opencsg/flows/" + contentID,
	}
	var buf io.Reader
	jsonData, err := json.Marshal(req)
	if err != nil {
	...
	buf = bytes.NewBuffer(jsonData)
	path := c.hc.endpoint + "/api/v1/opencsg/flows/" + contentID
	...
	hresp, err := c.hc.Do(hreq)
	if err != nil {
		slog.ErrorContext(ctx, "failed to update agent instance in agenthub", "error", err, "rpc_error_ctx", rpcErrorCtx)
		return errorx.RemoteSvcFail(errors.New("failed to update agent instance in agenthub"), rpcErrorCtx)
	}
	defer hresp.Body.Close()
	if hresp.StatusCode != http.StatusOK {
		slog.ErrorContext(ctx, "failed to update agent instance in agenthub", "status_code", hresp.StatusCode, "rpc_error_ctx", rpcErrorCtx)
		return errorx.RemoteSvcFail(errors.New("failed to update agent instance in agenthub"), rpcErrorCtx)
	}
...
func (c *AgentHubSvcClientImpl) RunAgentInstance(ctx context.Context, userUUID string, instanceID string, req *RunAgentInstanceRequest) (*RunAgentInstanceResponse, error) {
	...
	buf = bytes.NewBuffer(jsonData)
	path := c.hc.endpoint + "/api/v1/opencsg/run/" + instanceID + "?stream=false"
	hreq, err := http.NewRequestWithContext(ctx, http.MethodPost, path, buf)
	if err != nil {
		return nil, errorx.InternalServerError(err, rpcErrorCtx)
	}
	...
	hresp, err := c.hc.Do(hreq)
	if err != nil {
		slog.ErrorContext(ctx, "failed to run agent instance in agenthub", "error", err, "rpc_error_ctx", rpcErrorCtx)
		return nil, errorx.RemoteSvcFail(errors.New("failed to run agent instance in agenthub"), rpcErrorCtx)
	}
	defer hresp.Body.Close()
	if hresp.StatusCode != http.StatusOK {
		slog.ErrorContext(ctx, "failed to run agent instance in agenthub", "status_code", hresp.StatusCode, "rpc_error_ctx", rpcErrorCtx)
		return nil, errorx.RemoteSvcFail(errors.New("failed to run agent instance in agenthub"), rpcErrorCtx)
	}
...
func (c *AgentHubSvcClientImpl) RunAgentInstanceStream(ctx context.Context, userUUID string, contentID string, req *RunAgentInstanceRequest) (<-chan types.AgentStreamEvent, error) {
	...
	buf = bytes.NewBuffer(jsonData)
	path := c.hc.endpoint + "/api/v1/opencsg/run/" + contentID + "?stream=true"
	hreq, err := http.NewRequestWithContext(ctx, http.MethodPost, path, buf)
	if err != nil {
		return nil, errorx.InternalServerError(err, rpcErrorCtx)
	}
