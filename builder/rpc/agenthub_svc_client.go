...
func (c *AgentHubSvcClientImpl) UpdateAgentInstance(ctx context.Context, userUUID string, contentID string, req *UpdateAgentInstanceRequest) error {
	...
	hresp, err := c.hc.Do(hreq)
	if err != nil {
		return errorx.RemoteSvcFail(fmt.Errorf("failed to update agent instance in agenthub: %w", err), rpcErrorCtx)
	}
	defer hresp.Body.Close()
	if hresp.StatusCode != http.StatusOK {
		return errorx.RemoteSvcFail(fmt.Errorf("failed to update agent instance in agenthub, status code: %d", hresp.StatusCode), rpcErrorCtx)
	}
