{{/* title section */}}
{{if eq .event_type "create"}}您在 AgenticHub 提交的问题已创建{{else if eq .event_type "comment"}}您提交的 AgenticHub 问题 #{{.issue_id}} 收到新回复{{else}}您在AgenticHub提交的问题{{if .resolved_issue_version}}已于{{.resolved_issue_version}}版本解决{{else}}已解决{{end}}{{end}}
---
{{/* content section */}}
<html>
<body style="font-family: Arial, 'PingFang SC', 'Microsoft YaHei', sans-serif; line-height: 1.6; color: #333; max-width: 600px; margin: 0 auto; padding: 20px;">
	...
	<h2 style="color: #1e90ff; margin-top: 0; font-size: 18px;">您在 AgenticHub 提交的问题已创建</h2>
	...
	{{else}}
	<h2 style="color: #2e7d32; margin-top: 0; font-size: 18px;">您在 AgenticHub 提交的问题已解决</h2>
	<p style="margin-bottom: 16px;">感谢您的反馈，以下问题{{if .resolved_issue_version}}已于{{.resolved_issue_version}}版本解决{{else}}已解决{{end}}：</p>
	<div style="margin: 24px 0; padding: 16px; background-color: #f1f8e9; border-left: 4px solid #2e7d32; border-radius: 4px;">
		<p style="margin: 0 0 8px 0; font-size: 12px; color: #666;">问题 #{{.issue_id}}</p>
		<p style="margin: 0; font-weight: bold;">{{.issue_title}}</p>
	</div>
	<p style="margin-top: 20px;">感谢您对 AgenticHub 的贡献。</p>
	{{end}}

	<p style="margin-top: 32px; padding-top: 16px; border-top: 1px solid #eee; font-size: 12px; color: #888;">AgenticHub 团队</p>
</body>
</html>
