...
func TestAgentHandler_ListSessions(t *testing.T) {
	...
	t.Run("success with search parameter", func(t *testing.T) {
		tester := NewAgentServerHandlerTester(t).WithHandleFunc(func(h *AgentHandler) gin.HandlerFunc {
			return h.ListSessions
		})
		tester.WithKV("currentUserUUID", "u").WithParam("id", "16").WithQuery("search", "Test").WithQuery("per", "10").WithQuery("page", "1")
		...
		tester.mocks.agent.EXPECT().ListSessions(tester.Ctx(), "u", mock.MatchedBy(func(filter types.AgentInstanceSessionFilter) bool {
			return filter.InstanceID != nil && *filter.InstanceID == 16 && filter.Search == "Test"
		}), 10, 1).Return(expectedSessions, total, nil)
		tester.Execute()
...
func TestAgentHandler_ListTasks(t *testing.T) {
	t.Run("success - no filters", func(t *testing.T) {
		...
		tester.mocks.agent.EXPECT().ListTasks(tester.Ctx(), "u", mock.MatchedBy(func(filter types.AgentTaskFilter) bool {
			return filter.Search == "" && filter.TaskType == "" && filter.Status == "" && filter.InstanceID == nil && filter.SessionUUID == ""
		}), 10, 1).Return(expectedTasks, 1, nil)
		tester.Execute()
	...
	t.Run("success - with filters", func(t *testing.T) {
		...
		tester.WithKV("currentUserUUID", "u").
			WithQuery("search", "test").
			WithQuery("task_type", "finetuneJob").
			WithQuery("status", "in_progress").
			WithQuery("instance_id", "1").
			WithQuery("session_uuid", "session-1").
			WithQuery("per", "20").
			WithQuery("page", "2")
		...
		tester.mocks.agent.EXPECT().ListTasks(tester.Ctx(), "u", mock.MatchedBy(func(filter types.AgentTaskFilter) bool {
			return filter.Search == "test" &&
				filter.TaskType == types.AgentTaskTypeFinetuneJob &&
				filter.Status == types.AgentTaskStatusInProgress &&
				filter.InstanceID != nil && *filter.InstanceID == instanceID &&
				filter.SessionUUID == "session-1"
		}), 20, 2).Return(expectedTasks, 1, nil)
		tester.Execute()
	...
	t.Run("component error", func(t *testing.T) {
		...
		tester.mocks.agent.EXPECT().ListTasks(tester.Ctx(), "u", mock.Anything, 10, 1).Return(nil, 0, errors.New("database error"))
		tester.Execute()

		tester.ResponseEq(t, 500, "database error", nil)
	})

	t.Run("validation error from component", func(t *testing.T) {
		tester := NewAgentServerHandlerTester(t).WithHandleFunc(func(h *AgentHandler) gin.HandlerFunc {
			return h.ListTasks
		})
		tester.WithKV("currentUserUUID", "u").
		...
		tester.mocks.agent.EXPECT().ListTasks(tester.Ctx(), "u", mock.Anything, 10, 1).Return(nil, 0, errors.New("instance_id is required when session_uuid is provided"))
		tester.Execute()

		tester.ResponseEq(t, 500, "instance_id is required when session_uuid is provided", nil)
	})

	t.Run("forbidden error from component", func(t *testing.T) {
		tester := NewAgentServerHandlerTester(t).WithHandleFunc(func(h *AgentHandler) gin.HandlerFunc {
			return h.ListTasks
		})
		tester.WithKV("currentUserUUID", "u").
			WithQuery("instance_id", "999").
			WithQuery("per", "10").
			WithQuery("page", "1")

		tester.mocks.agent.EXPECT().ListTasks(tester.Ctx(), "u", mock.Anything, 10, 1).Return(nil, 0, errorx.ErrForbidden)
		tester.Execute()
...
func TestAgentHandler_GetTaskDetail(t *testing.T) {
	t.Run("success", func(t *testing.T) {
		tester := NewAgentServerHandlerTester(t).WithHandleFunc(func(h *AgentHandler) gin.HandlerFunc {
			return h.GetTaskDetail
		})
		tester.WithKV("currentUserUUID", "u").WithParam("id", "1")
		...
		tester.mocks.agent.EXPECT().GetTaskDetail(tester.Ctx(), "u", int64(1)).Return(expectedDetail, nil)
		tester.Execute()

		tester.ResponseEq(t, 200, tester.OKText, expectedDetail)
	})
	...
	t.Run("task not found", func(t *testing.T) {
		...
		tester.WithKV("currentUserUUID", "u").WithParam("id", "999")

		tester.mocks.agent.EXPECT().GetTaskDetail(tester.Ctx(), "u", int64(999)).Return(nil, errorx.ErrDatabaseNoRows)
		tester.Execute()
	...
	t.Run("component error", func(t *testing.T) {
		...
		tester.mocks.agent.EXPECT().GetTaskDetail(tester.Ctx(), "u", int64(1)).Return(nil, errors.New("database error"))
		tester.Execute()
