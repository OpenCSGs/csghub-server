...
`
	...
	case int:
		return int64(v), nil
	default:
		return 0, fmt.Errorf("unexpected result type from Redis script: %T (value: %v)", res, res)
	}
}
...
func RateLimiter(config *config.Config, opt ...RateLimiterOption) gin.HandlerFunc {
	...
	return func(c *gin.Context) {
		...
		resetTime := time.Now().Add(time.Duration(o.rateLimitConfig.Window) * time.Second).Unix()
