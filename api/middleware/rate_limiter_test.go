...
func TestRateLimiter_Exceeded_AbortCustomCallback(t *testing.T) {
	gin.SetMode(gin.TestMode)
	r := gin.New()
	...
	customCallbackRun := false
	customCallback := func(c *gin.Context) {
		customCallbackRun = true
		c.JSON(http.StatusTooManyRequests, gin.H{"error": "custom message"})
	}

	r.Use(RateLimiter(cfg, WithLimiterAlgorithm(mockAlg), WithOnLimitExceeded(customCallback)))

	mainHandlerRun := false
	r.GET("/test", func(c *gin.Context) {
		mainHandlerRun = true
		c.String(http.StatusOK, "OK")
	})
	...
	assert.Equal(t, http.StatusTooManyRequests, w.Code)
	assert.True(t, customCallbackRun)
	assert.False(t, mainHandlerRun)
}

func TestSlidingWindowLimiter_UnexpectedType(t *testing.T) {
	mockRedis := mock_cache.NewMockRedisClient(t)
	limiter := &slidingWindowLimiter{
		redisClient: mockRedis,
		window:      60,
	...
	mockRedis.EXPECT().
		RunScript(ctx, mock.Anything, []string{"rate_limit:sw:test_action:test_user"}, mock.Anything, mock.Anything, int64(10), mock.Anything).
		Return("unexpected_string", nil).
		Once()

	_, err := limiter.Check(ctx, "test_action", "test_user", 10)
	assert.Error(t, err)
	assert.Contains(t, err.Error(), "unexpected result type from Redis script")
}

func TestTokenBucketLimiter_UnexpectedType(t *testing.T) {
	mockRedis := mock_cache.NewMockRedisClient(t)
	limiter := &tokenBucketLimiter{
		redisClient: mockRedis,
		window:      60,
