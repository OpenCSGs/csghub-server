...
func HandlePushWorkflow(ctx workflow.Context, req *types.GiteaCallbackPushReq) error {
	...
	// Watch space change
	err := workflow.ExecuteActivity(ctx, activities.WatchSpaceChange, req).Get(ctx, nil)
	if err != nil {
	...
	// Watch repo relation
	err = workflow.ExecuteActivity(ctx, activities.WatchRepoRelation, req).Get(ctx, nil)
	if err != nil {
	...
	err = workflow.ExecuteActivity(ctx, activities.SetRepoUpdateTime, req).Get(ctx, nil)
	if err != nil {
	...
	// Update repo infos
	err = workflow.ExecuteActivity(ctx, activities.UpdateRepoInfos, req).Get(ctx, nil)
	if err != nil {
	...
	// Sensitive check
	err = workflow.ExecuteActivity(ctx, activities.SensitiveCheck, req).Get(ctx, nil)
	if err != nil {
