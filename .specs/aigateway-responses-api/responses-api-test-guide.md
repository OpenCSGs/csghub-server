...
## 13. Adapter Reasoning Forwarding (opt-in per upstream)

Adapter mode ignores `reasoning` unless the selected upstream declares:

```text
metadata.responses.chat_adapter.reasoning_request
```

Example upstream metadata for DeepSeek-style thinking control:
  ...
  "responses": {
    "chat_adapter": {
      "reasoning_request": {
        "enabled": true,
        "effort_field": "",
        "enable_extra": { "thinking": { "type": "enabled" } },
        "disable_extra": { "thinking": { "type": "disabled" } }
      }
    }
  }
}
...
Expected when upstream metadata is configured: no `reasoning` output item in the response (upstream thinking disabled).

Unsupported reasoning on a small-model upstream (`enabled: false`):
