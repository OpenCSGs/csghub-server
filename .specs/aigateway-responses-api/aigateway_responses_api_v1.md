...
- AIGateway exposes the OpenAI Responses API as its public contract. Upstream metadata defines how the standard `reasoning` object is translated into provider-specific chat request fields. The generic chat adapter itself remains provider-agnostic.
...
- `reasoning` is forwarded in adapter mode only when the selected upstream declares `metadata.responses.chat_adapter.reasoning_request`. When absent, `reasoning` is ignored (current behavior). When `enabled: false`, `reasoning.effort: none` is dropped and other effort values return `unsupported_feature:reasoning`. When `enabled: true`, `reasoning.effort` is translated into upstream chat fields per upstream config (`effort_field`, `enable_extra`, `disable_extra`). An empty `effort_field` means merge-only (no effort key written).
