...
- `audio` file, repeated once per speaker where supported
- `audio_type`, `num_segments`, `ref_img_index`, `mask_frame_range`, and `bbox` for LongCat Avatar
...
LongCat Video Avatar models require multipart input with one or two `audio`
files. One audio supports audio-text-to-video and, with `input_reference`,
audio-image-text-to-video. Two audio files require `input_reference` and map
in order to `person1` and `person2`; `audio_type` is `para` or `add`. LongCat
accepts only `832x480` and `1280x768`; use `num_segments` instead of
`seconds` for continuation. `bbox` is JSON whose coordinates use
`[ymin,xmin,ymax,xmax]`.
