...
- text-to-video
- image-to-video
- audio-driven avatar video for compatible models
...
The same operations are also available through the
`/v1/video/generations` compatibility route.
...
## Example 4: LongCat Audio-Driven Avatar

LongCat Avatar requires one or two audio uploads. Repeat `audio` in speaker
order for two-person generation, which also requires an image:

```bash
curl -X POST "$AIGATEWAY_BASE_URL/v1/video/generations" \
  -H "Authorization: Bearer $AIGATEWAY_API_KEY" \
  -F "model=meituan-longcat/LongCat-Video-Avatar-1.5" \
  -F "prompt=Two presenters talk in a recording studio" \
  -F "size=832x480" \
  -F "audio=@person1.wav;type=audio/wav" \
  -F "audio=@person2.wav;type=audio/wav" \
  -F "input_reference=@presenters.png;type=image/png" \
  -F "audio_type=para" \
  -F "num_segments=5" \
  -F 'bbox={"person1":[40,20,440,400],"person2":[40,430,440,810]}'
```

For a single-person audio-text request, upload one `audio` and omit
`input_reference`. LongCat supports `832x480` and `1280x768`; it uses
`num_segments` for continuation and rejects arbitrary `seconds`.
