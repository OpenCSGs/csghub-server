...
| `content_id` | Unique identifier returned by the backing service. For `llmwiki` it also serves as the KB's MCP server name, so it must match the MCP server name format (SEP-986: leading letter/digit, then letters, digits, `_`, `-`; max 32 chars). |
...
An LLM-Wiki KB is created in one of two modes. In registry mode the caller supplies an existing KB's `kb_id` as `content_id` and its private per-KB MCP service URL in `metadata.mcp_endpoint_url`; CSGHub stores that private URL for authorized AIGateway routing and does not call the LLM-Wiki backend. In auto-provision mode `metadata.mcp_endpoint_url` is omitted: CSGHub creates the KB on the LLM-Wiki backend with the request `content_id` as `kb_id`, stamps the record metadata with `backend_managed: true`, and stores the backend-returned MCP endpoint when it is routable (absolute `http` URL, path exactly `/mcp`, no credentials/query/fragment). `content_id` is globally unique; a duplicate is rejected before provisioning, and create-failure compensation re-checks content-ID ownership before deleting a backend resource so it never removes a resource owned by another record. Deleting a `backend_managed` record — by numeric ID or by content ID — deletes the LLM-Wiki backend resource; deleting a registry record never touches the backend.
...
Metadata updates are partial. `base_url`, `resource_state`, and `backend_managed` are response-only. An administrator may replace the MCP target with another valid private endpoint; returning the exact public `metadata.mcp_endpoint_url` for the same KB is normalized to a no-op so the stored private target is not overwritten. Other public or cross-KB URLs remain invalid. The endpoint may also be removed; the record stays readable and its MCP route returns `503` until an endpoint is configured again.
