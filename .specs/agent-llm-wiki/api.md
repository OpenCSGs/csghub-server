...
An LLM-Wiki KB is created in one of two modes:

- **Registry** — the request carries `metadata.mcp_endpoint_url`. The LLM-Wiki KB and its MCP service already exist; creating the Agent KB record only registers them locally and does not call the LLM-Wiki backend.
- **Auto-provision** — the request omits `metadata.mcp_endpoint_url`. CSGHub provisions a real KB on the LLM-Wiki backend (`POST /internal/v1/knowledge-bases`) using the request `content_id` as the backend `kb_id`, marks the record `backend_managed` in metadata, and persists the backend-returned `mcp_endpoint_url` when the response contains a routable one.
...
`content_id` must be the target LLM-Wiki `kb_id` and is globally unique across knowledge bases; a create with an already-used `content_id` is rejected before any backend call. Because the value also serves as the KB's MCP server name, it must follow the MCP server name format: start with a letter or digit, contain only letters, digits, underscores, or hyphens, and be at most 32 characters (`AGENT-ERR-28` otherwise).

`metadata.mcp_endpoint_url` is optional. When supplied (registry mode), the value must be an absolute `http` URL whose path is exactly `/mcp`, without credentials, query parameters, or a fragment. CSGHub stores this private address but replaces it with the public AIGateway route in API responses. When omitted, the auto-provisioned backend URL is stored instead if it follows the same constraints; an unroutable or not-yet-provisioned URL is left unset, and the KB's MCP route returns `503` until the endpoint is known.
...
These operations update or remove the CSGHub Agent KB record. Updates never touch the LLM-Wiki resource. Deletes behave per mode: deleting a registry KB removes only the local record, while deleting an auto-provisioned (`backend_managed`) KB also deletes the LLM-Wiki backend resource — on both the ID and content-ID routes. LLM-Wiki content changes use the management proxy.

`metadata` updates are partial. Clients should omit response-only `base_url` and `resource_state` fields; `backend_managed` is backend-owned and rejected on write. An administrator may replace `metadata.mcp_endpoint_url` through either update route by supplying a private endpoint that follows the registration constraints. If a client sends back the exact public MCP URL returned for the same KB, CSGHub treats it as unchanged and preserves the stored private endpoint. A different public or cross-KB URL is rejected. The endpoint may be removed from a record; records without this metadata remain readable, but their MCP route returns `503` until an endpoint is configured.
