...
const (
	...
	codeSandboxCrossClusterRestartErr        // restart targets a different cluster
)
...
var (
	...
	// Description: A sandbox cannot be restarted onto a resource in a different cluster because its persistent data is cluster-scoped.
	//
	// Description_ZH: 沙箱不能跨集群重启：持久化数据仅存在于原集群
	//
	// en-US: A sandbox cannot be restarted onto a resource in a different cluster because its persistent data is cluster-scoped.
	//
	// zh-CN: 沙箱不能跨集群重启：持久化数据仅存在于原集群
	//
	// zh-HK: 沙箱不能跨集群重啟：持久化數據僅存在於原集群
	ErrSandboxCrossClusterRestart error = CustomError{prefix: errSandboxPrefix, code: codeSandboxCrossClusterRestartErr}
)
