...
type PDPlanResult struct {
	...
	DP int
	// TotalGPUs is the total number of GPUs per LWS group (TP, since DP=1 and EP follows TP).
	TotalGPUs int
...
type PDPlanInput struct {
	...
	// MaxTotalGPUs limits the TP degree and thus total GPUs per LWS group.
	// 0 means use default (1024). Since TotalGPUs = TP, this controls the maximum
	// TP candidate generated by the planner. Multi-node TP (TP > GPUsPerNode) is
	// used as a fallback for large models.
	MaxTotalGPUs int
}
