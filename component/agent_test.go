...
func createTestAdapterFactory(mockClient rpc.AgentHubSvcClient) *AgentInstanceAdapterFactory {
	...
	langflowAdapter := NewLangflowAgentInstanceAdapterWithClient(mockClient)
...
func TestAgentComponent_ListSessions(t *testing.T) {
	...
	t.Run("success", func(t *testing.T) {
		...
		// Setup mock expectations
		ac.mocks.sessionStore.EXPECT().List(ctx, mock.Anything, 10, 1).Return(dbSessions, 2, nil)
	...
	t.Run("database error", func(t *testing.T) {
		...
		// Setup mock expectations
		ac.mocks.sessionStore.EXPECT().List(ctx, mock.Anything, 10, 1).Return(nil, 0, errors.New("database error"))
