...
func (c *repoComponentImpl) DeletePendingDeletion(ctx context.Context) error {
	var batch int
	batchSize := 1000
	for {
		pds, err := c.pendingDeletion.FindByTableNameWithBatch(ctx, database.PendingDeletionTableNameRepository, batchSize, batch)
		...
		for _, pd := range pds {
			err := c.git.DeleteRepo(ctx, pd.Value)
			if err != nil {
			...
			err = c.pendingDeletion.Delete(ctx, pd)
			if err != nil {
