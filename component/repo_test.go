...
func TestRepoComponent_PublicToUser(t *testing.T) {
	...
	repo.mocks.stores.RepoMock().EXPECT().PublicToUser(ctx, types.ModelRepo, []int64{1, 2, 3}, filter, 10, 1).Return(mrepos, 100, nil)
...
// 					ctx, repo.lfsBucket, "lfs/path", nil, reqParams,
...
// 	f, e := repo.HeadDownloadFile(ctx, &types.GetFileReq{
// 		Namespace: "ns",
// 		Name:      "n",
// 		Ref:       "main",
// 		Path:      "path",
...
// 				reqParams.Set("response-content-disposition", fmt.Sprintf("attachment;filename=%s", "zzz"))
// 				repo.mocks.s3Client.EXPECT().PresignedGetObject(
// 					ctx, repo.lfsBucket, "lfs/qqq", nil, reqParams,
// 				).Return(&url.URL{Path: "foobar"}, nil)
// 			} else {
// 				repo.mocks.gitServer.EXPECT().GetRepoFileReader(ctx, gitserver.GetRepoInfoByPathReq{
// 					Namespace: "ns",
...
// 			repo.mocks.stores.AccessTokenMock().EXPECT().GetUserGitToken(ctx, "user").Return(&database.AccessToken{}, nil)
...
// 			if c.saas {
// 				if c.gitea {
// 					repo.mocks.mirrorServer.EXPECT().CreateMirrorRepo(ctx, mirrorserver.CreateMirrorRepoReq{
// 						Namespace:   "root",
// 						Name:        "_model_ns_n",
// 						CloneUrl:    "su",
// 						Username:    "user",
...
// 			} else {
// 				if c.gitea {
// 					repo.mocks.gitServer.EXPECT().MirrorSync(ctx, gitserver.MirrorSyncReq{
// 						Namespace:   "ns",
// 						Name:        "n",
// 						CloneUrl:    "su",
// 						Username:    "user",
...
// 				}).Return()
// 				repo.mocks.stores.MirrorMock().EXPECT().Update(ctx, &rmi).Return(nil)
// 			}
...
// 			}
// 			if c.mirror {
// 				repo.mocks.stores.MirrorMock().EXPECT().FindByRepoID(ctx, int64(123)).Return(&database.Mirror{}, nil)
// 			} else {
// 				repo.mocks.stores.MirrorMock().EXPECT().FindByRepoID(ctx, int64(123)).Return(nil, sql.ErrNoRows)
// 				repo.mocks.stores.SyncVersionMock().EXPECT().FindByRepoTypeAndPath(ctx, "", types.ModelRepo).Return(&database.SyncVersion{SourceID: types.SyncVersionSourceHF}, nil)
...
// 					RepoType:    types.ModelRepo,
// 				}).Return(0, nil)
// 				repo.mocks.stores.MirrorMock().EXPECT().Create(ctx, mirror).Return(nil, nil)
...
// 				mockedRepo, nil,
// 			)
// 			repo.mocks.stores.SyncClientSettingMock().EXPECT().First(ctx).Return(&database.SyncClientSetting{
// 				Token: "foo",
// 			}, nil)
...
// 					}).Return(nil)
// 				}
// 				mockedRepo.SyncStatus = types.SyncStatusInProgress
// 			} else {
...
// 	repo.mocks.stores.SpaceResourceMock().EXPECT().FindByID(ctx, int64(123)).Return(&database.SpaceResource{
// 		ID: 111,
// 	}, nil)
