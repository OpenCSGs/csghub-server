...
func (c *agentConfigComponentImpl) Create(ctx context.Context, req *types.CreateAgentConfigReq) (*types.AgentConfig, error) {
	if strings.HasPrefix(req.Name, "sandbox_runtime.") {
		return nil, fmt.Errorf("sandbox runtime profiles are managed from Git configuration")
	}
...
func (c *agentConfigComponentImpl) Delete(ctx context.Context, id int64) error {
	config, err := c.agentConfigStore.GetByID(ctx, id)
	if err != nil {
		return fmt.Errorf("failed to get agent config: %w", err)
	}
	if config != nil && strings.HasPrefix(config.Name, "sandbox_runtime.") {
		return fmt.Errorf("sandbox runtime profiles are managed from Git configuration")
	}
...
func (c *agentConfigComponentImpl) Update(ctx context.Context, id int64, req *types.UpdateAgentConfigReq) (*types.AgentConfig, error) {
	...
	if strings.HasPrefix(dbConfig.Name, "sandbox_runtime.") {
		return nil, fmt.Errorf("sandbox runtime profiles are managed from Git configuration")
	}
