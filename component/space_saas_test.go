...
func TestSpaceComponent_Update(t *testing.T) {
	...
	sc.mocks.components.accounting.EXPECT().QueryPricesBySKUType("", types.AcctPriceListReq{
	...
	sc.mocks.components.accounting.EXPECT().QueryBalanceByUserIDInternal(ctx, "user").Return(&database.AccountUser{
		Balance: 5,
	}, nil)
...
func TestSpaceComponent_UpdateReserved(t *testing.T) {
	ctx := context.TODO()
	sc := initializeTestSpaceComponent(ctx, t)
	...
	require.Equal(t, "don't support switch reserved resource so far", err.Error())

}

func TestSpaceComponent_StopEE(t *testing.T) {
	ctx := context.TODO()
	sc := initializeTestSpaceComponent(ctx, t)
	...
	sc.mocks.stores.DeployTaskMock().EXPECT().GetLatestDeployBySpaceID(ctx, int64(1)).Return(
		&database.Deploy{
			SvcName: "svc", RepoID: 1, UserID: 2, ID: 3,
			OrderDetailID: 123,
		}, nil,
	)
