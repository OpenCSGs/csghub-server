...
func NewCounterFactory() CounterFactory {
	return &tokenizerFactoryImpl{}
}

type tokenizerFactoryImpl struct{}

func (f *tokenizerFactoryImpl) NewChat(param CreateParam) ChatTokenCounter {
	tokenizer := NewTokenizerImpl(param.Endpoint, param.Host, param.Model, param.ImageID)
	return NewLLMTokenCounter(tokenizer)
}

func (f *tokenizerFactoryImpl) NewEmbedding(param CreateParam) *EmbeddingTokenCounter {
	tokenizer := NewTokenizerImpl(param.Endpoint, param.Host, param.Model, param.ImageID)
